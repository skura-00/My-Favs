        ...
        NavigationSplitView {
            ...
            ZStack {
                if let item = selection, let itemBinding = favItemData.getBindingToData(item) {
                    FavListView(favItemData: favItemData, favCategory: itemBinding)
                } else {
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        FavCategoryView(favItemData: FavSampleData()).environment(\.managedObjectContext, PersistenceController.preview.container.viewContext)
    }
}
